...
def _sw_preflight(drone):
    ...
    out=run["stdout"]; block=out.split("__T__",1); block=block[1].split("__E__",1)[0] if len(block)==2 else ""; typ=out.split("__Y__",1); typ=typ[1].split("__Z__",1)[0].strip() if len(typ)==2 else ""
    result["visible_topics"]=_sw_topics(block); result["vehicle_status_type"]=typ
    ...
    if "px4_msgs" not in typ: result["errors"].append("Drone ROS preflight could not confirm px4_msgs decoding")
...
def _sw_worker(args):
    ...
    status_path=status_dir/"worker_status.json"; final_path=meta/"worker_final.json"; bag=root/"ros_bags"/f"{args.mission_id}__{args.drone_id}_ground_rosbag"
    ...
    topics_run=_sw_run(["ros2","topic","list","-t"],20,os.environ.copy()); topics=_sw_topics(topics_run.get("stdout","")); types=_sw_types(topics_run.get("stdout","")); status_type=types.get(_SW_STATUS,"") or _sw_run(["ros2","topic","type",_SW_STATUS],env=os.environ.copy()).get("stdout",""); vehicle=_sw_echo(_SW_STATUS,8); land_available=_SW_LAND in topics and "px4_msgs" in types.get(_SW_LAND,"")
    ...
    ground={"visible_topics":topics,"topic_types":types,"vehicle_status_type":status_type,"vehicle_status_echo":vehicle,"vehicle_land_detected_available":land_available,"warnings":pre_warnings,"errors":pre_errors}
    if pre_errors:
    ...
    save_json_atomic(status_path,{"mission_id":args.mission_id,"drone_id":args.drone_id,"ip_address":args.ip_address,"ros_domain_id":args.ros_domain_id,"static_peer":args.ip_address,"state":"READY","phase":"READY","bag":{"prepared":True,"path":str(bag)},"audio":{"enabled":args.enable_audio,"started":False}})
